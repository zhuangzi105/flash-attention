...
template<typename Kernel_traits, bool Is_dropout, bool Is_causal, bool Is_even_MN, bool Is_even_K, bool Is_first, bool Is_last, bool Seq_parallel=false, typename Params>
__forceinline__ __device__ void compute_dq_dk_dv_1colblock_densemask(const Params &params, const int bidb, const int bidh, const int n_block) {
...
template<typename Kernel_traits, bool Is_dropout, bool Is_causal, bool Is_even_MN, bool Is_even_K, bool Is_densemask, bool Is_deterministic, bool Is_flashmask, typename Params>
    ...
    if (Is_densemask) {
        if (Is_deterministic) {  // params.num_splits == 1, means grid.x = 1, blockIdx.x = 0;
            ...
            for(int i = 0; i < params.seqlen_k; i+= kBlockN) {
               compute_dq_dk_dv_1colblock_densemask<Kernel_traits, Is_dropout, Is_causal, Is_even_MN, Is_even_K, false, false, /*Seq_parallel=*/true>(params, bidb, bidh, loop_step_x);
                loop_step_x += 1;
            }
        } else {
            compute_dq_dk_dv_1colblock_densemask<Kernel_traits, Is_dropout, Is_causal, Is_even_MN, Is_even_K, false, false, /*Seq_parallel=*/true>(params, bidb, bidh, n_block);
        }
    } else if (Is_flashmask) {
        if (Is_deterministic) {  // params.num_splits == 1, means grid.x = 1, blockIdx.x = 0;
            int loop_step_x = 0;
            for(int i = 0; i < params.seqlen_k; i+= kBlockN) {
               compute_dq_dk_dv_1colblock_flashmask<Kernel_traits, Is_dropout, Is_causal, Is_even_MN, Is_even_K, false, false, /*Seq_parallel=*/true>(params, bidb, bidh, loop_step_x);
                loop_step_x += 1;
            }
        } else {
            compute_dq_dk_dv_1colblock_flashmask<Kernel_traits, Is_dropout, Is_causal, Is_even_MN, Is_even_K, false, false, /*Seq_parallel=*/true>(params, bidb, bidh, n_block);
        }
    } else {
        if (Is_deterministic) {  // params.num_splits == 1, means grid.x = 1, blockIdx.x = 0;
            int loop_step_x = 0;
