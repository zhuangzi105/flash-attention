...
struct CollectiveMainloopBwdSm90 {
    ...
    struct Arguments {
        ...
        // PaddleBox: bias, mask, dbias for backward
    ...
    mma(Params const& params,
        ...
        auto bwd_step = [&](int m_block, auto mask_fn) {
            ...
            // PaddleBox: add compact varlen bias (layout=2) to recomputed QK scores
            if (params.ptr_bias != nullptr) {
                ...
                Tensor tScS_b_rc = make_tensor(tScS_b.data(),
                    flash::convert_layout_acc_rowcol<SdP_swapAB>(tScS_b.layout()));
            ...
            // PaddleBox: add explicit attention mask (additive, padded rectangular layout)
            if (params.ptr_mask != nullptr) {
                ...
                Tensor tScS_m_rc = make_tensor(tScS_m.data(),
                    flash::convert_layout_acc_rowcol<SdP_swapAB>(tScS_m.layout()));
            ...
            // PaddleBox: write dS (= dBias) to global memory in compact varlen layout
