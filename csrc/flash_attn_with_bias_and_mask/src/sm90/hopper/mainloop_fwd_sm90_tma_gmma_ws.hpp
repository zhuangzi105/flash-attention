...
struct CollectiveMainloopFwdSm90 {
    ...
    struct Arguments {
        ...
        // PaddleBox: compact varlen bias (layout=2) support
        void const* const ptr_bias = nullptr;           // bias buffer, same dtype as Element
        int const* const bias_seq_offsets = nullptr;     // prefix sum [b+1], in elements
        // PaddleBox: explicit attention mask (padded rectangular layout from SM80)
        void const* const ptr_mask = nullptr;            // mask buffer, same dtype as Element
        int mask_head_mod_size = 0;                      // head broadcasting dimension
        int mask_seq_mod_size = 0;                       // query row broadcasting dimension
        int mask_row_stride = 0;                         // physical last dim (= max_seqlen_k)
    };
    ...
    struct Params {
        ...
        int const *const seqlens_rotary = nullptr;
        // PaddleBox: compact varlen bias (layout=2) support
        void const* const ptr_bias = nullptr;
        int const* const bias_seq_offsets = nullptr;
        // PaddleBox: explicit attention mask (padded rectangular layout from SM80)
        void const* const ptr_mask = nullptr;
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        return {args.ptr_Q, args.shape_Q, args.stride_Q, shape_Q_packed, stride_Q_packed,
                ...
                args.ptr_bias, args.bias_seq_offsets,
                args.ptr_mask, args.mask_head_mod_size, args.mask_seq_mod_size, args.mask_row_stride};
    }
    ...
    mma(Params const& params,
        ...
        auto scoremod_premask_fn = [&](auto& tSrS) {
            ...
            // PaddleBox: add compact varlen bias (layout=2) to QK scores before masking
            if (params.ptr_bias != nullptr) {
                // Use same identity tensor + partition_C pattern as flash::Mask
                // to get per-thread (m, n) coordinates within the tile
                Tensor cS = cute::make_identity_tensor(Shape<Int<kBlockM>, Int<kBlockN>>{});
                auto thread_mma_qk = TiledMmaQK{}.get_thread_slice(thread_idx);
                ...
                // Compact varlen bias address: bias_ptr + bias_seq_offsets[bidb] + bidh * T_i^2 + row * T_i + col
                // T_i = seqlen_q (= seqlen_k for self-attention in PaddleBox/AlphaFold2)
                int const T_i = seqlen_q;
                Element const* bias_head_ptr = reinterpret_cast<Element const*>(params.ptr_bias)
                    + params.bias_seq_offsets[bidb] + bidh * T_i * T_i;
                #pragma unroll
                for (int m = 0; m < size<0>(tSrS_rowcol); ++m) {
                    int const row = get<0>(tScS_rowcol(m, _0{})) + m_block * kBlockM;
                    if (row < T_i) {
                        #pragma unroll
                        for (int n_idx = 0; n_idx < size<1>(tSrS_rowcol); ++n_idx) {
                            int const col = get<1>(tScS_rowcol(_0{}, n_idx)) + n_block * kBlockN;
                            if (col < T_i) {
                                tSrS_rowcol(m, n_idx) += static_cast<float>(bias_head_ptr[row * T_i + col]);
                            }
                        }
                    }
                }
            }
            // PaddleBox: add explicit attention mask (additive, padded rectangular layout) before causal masking
            // SM80 order: mask(additive) → bias(additive) → causal(-inf). Same order here.
            // Mask layout: [batch * mask_head_mod_size, mask_seq_mod_size, mask_row_stride]
            if (params.ptr_mask != nullptr) {
                Tensor cS_m = cute::make_identity_tensor(Shape<Int<kBlockM>, Int<kBlockN>>{});
                auto thread_mma_qk_m = TiledMmaQK{}.get_thread_slice(thread_idx);
                Tensor tScS_m = thread_mma_qk_m.partition_C(cS_m);
                Tensor tScS_m_rowcol = make_tensor(tScS_m.data(), flash::convert_layout_acc_rowcol(tScS_m.layout()));
                Tensor tSrS_m_rowcol = make_tensor(tSrS.data(), flash::convert_layout_acc_rowcol(tSrS.layout()));
                int const mhs = params.mask_head_mod_size;
                int const mss = params.mask_seq_mod_size;
                int const mrs = params.mask_row_stride;
                Element const* mask_base_ptr = reinterpret_cast<Element const*>(params.ptr_mask)
                    + (bidb * mhs + (bidh % mhs)) * mss * mrs;
                #pragma unroll
                for (int m = 0; m < size<0>(tSrS_m_rowcol); ++m) {
                    int const row = get<0>(tScS_m_rowcol(m, _0{})) + m_block * kBlockM;
                    if (row < seqlen_q) {
                        int const mask_row = row % mss;
                        #pragma unroll
                        for (int n_idx = 0; n_idx < size<1>(tSrS_m_rowcol); ++n_idx) {
                            int const col = get<1>(tScS_m_rowcol(_0{}, n_idx)) + n_block * kBlockN;
                            if (col < seqlen_k) {
                                tSrS_m_rowcol(m, n_idx) += static_cast<float>(
