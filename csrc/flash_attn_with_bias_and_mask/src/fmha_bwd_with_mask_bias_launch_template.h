...
bool run_fmha_dgrad_fp16_sm80_loop_(const FMHA_dgrad_params &params, cudaStream_t stream) {
    ...
    BOOL_SWITCH_FUNC(params.is_causal, IsCausalConst, [&] {
        BOOL_SWITCH_FUNC(is_dropout, IsDropoutConst, [&] {
            auto kernel = &fmha_dgrad_fp16_sm80_dq_dk_dv_loop_kernel<Kernel_traits, IsDropoutConst, IsCausalConst>;
            if (params.seqlen_k == blocksize_c) {
                kernel = &fmha_dgrad_fp16_sm80_dq_dk_dv_loop_kernel<Kernel_traits, IsDropoutConst, IsCausalConst, /*loop_steps=*/1>;
            } else if (params.seqlen_k == blocksize_c * 2) {
                kernel = &fmha_dgrad_fp16_sm80_dq_dk_dv_loop_kernel<Kernel_traits, IsDropoutConst, IsCausalConst, /*loop_steps=*/2>;
            }
